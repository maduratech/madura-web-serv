/** Per-person INR rates (twin = double sharing). */
...
export const DEFAULT_CHILD_BED_AGE_MIN = 11;
...
export function normalizePriceSheet(sheet: TourPriceSheet): {
  ...
  const childBed = Number(sheet.child_with_bed_price) || 0;
  const childNoBed = Number(sheet.child_without_bed_price) || 0;
  return {
    twin_sharing_price: twin,
    triple_sharing_price: triple,
    single_sharing_price: single,
    child_with_bed_price: childBed,
    child_without_bed_price: childNoBed,
  };
}
...
export function childPerPersonRate(
  age: number,
  sheet: TourPriceSheet,
  childBedAgeMin = DEFAULT_CHILD_BED_AGE_MIN
): number {
  const s = normalizePriceSheet(sheet);
  const a = Number.isFinite(age) ? age : childBedAgeMin;
  if (a >= childBedAgeMin) {
    return s.child_with_bed_price || s.child_without_bed_price || 0;
  }
  return s.child_without_bed_price || s.child_with_bed_price || 0;
}
...
export function computeBookingTotalInr(input: {
  ...
  const childBedAgeMin = input.childBedAgeMin ?? DEFAULT_CHILD_BED_AGE_MIN;
  ...
  for (const room of rooms) {
    ...
    for (let i = 0; i < room.children; i += 1) {
      const age = ages[i] ?? childBedAgeMin;
      total += applyDiscountPercent(childPerPersonRate(age, input.sheet, childBedAgeMin), discount);
    }
  }
